...
const ProjectItem = (props: ProjectItemProps) => {
  ...
  return (
    <div className={[styles.projectItem, inView ? styles.visible : ''].join(' ')} ref={ref}>
      <div className={'flex flex-col gap-[30px]'}>
        <div className={'flex flex-col gap-4'}>
          <div className={'flex items-center gap-4 text-xl font-bold text-white'}>
            ...
            {websiteLink && (
              <Link href={websiteLink} title={websiteLink} rel={'noopener noreferrer'} target={'_blank'} className={'ml-auto'}>
                <ExternalLink />
              </Link>
            )}
          </div>
          <div className={'text-md max-w-[500px] tracking-wide text-white/[60%]'}>{description}</div>
        </div>
        <TechnologyGrid accentColor={'rgb(208,208,208)'} technologies={technologies} borderRadius={5} />
      </div>
    </div>
  )
}
